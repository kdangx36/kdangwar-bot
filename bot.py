...
def xử_lý_thêm_acc(message):
    ...
    try:
        # Định dạng: Giá|Thông tin nick
        chuoi = message.text.split("|")
        gia = int(chuoi[0].strip())
        thong_tin = chuoi[1].strip()
        
        data = doc_data()
        # Tìm ID lớn nhất hiện tại để tăng dần liên tục không trùng lặp
        max_id = max([acc["id"] for acc in data["accounts"]] if data["accounts"] else [0])
        new_id = max_id + 1
        ...
        ghi_data(data)
        bot.reply_to(message, f"✅ Đã thêm thành công sản phẩm *ACC #{new_id}* vào kho hàng!", parse_mode="Markdown")
    except Exception as e:
        bot.reply_to(message, "❌ Định dạng nhập vào bị sai! Hãy chắc chắn bạn đã nhập đúng mẫu `Giá | Thông tin` (Ví dụ: `20000 | TK: dang123 | MK: 123`)")
# ====================================================================
...
def callback_xu_ly(call):
    ...
    if call.data == "admin_add_acc":
        ...
        for acc in data["accounts"]:
            if acc["status"] == "ConHang":
                co_acc = True
                markup.add(types.InlineKeyboardButton(f"❌ Xoá Acc #{acc['id']} - Giám: {acc['price']:,}đ", callback_data=f"adm_delete_{acc['id']}"))
