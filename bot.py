...
if not os.path.exists(DATA_FILE):
    initial_data = {
        "users": {},
        "accounts": []  # Để trống hoàn toàn để khi thêm sẽ bắt đầu từ #1
    }
...
def admin_panel(message):
    ...
    btn_add = types.InlineKeyboardButton("➕ THÊM ACC", callback_data="admin_add_acc")
    btn_del = types.InlineKeyboardButton("➖ XOÁ ACC", callback_data="admin_del_acc")
...
def xử_lý_thêm_acc(message):
    ...
    try:
        if "|" not in message.text:
            bot.reply_to(message, "❌ Sai định dạng! Bạn phải dùng dấu gạch đứng `|` để ngăn cách.\nVí dụ: `20000 | TK: abc | MK: 123`")
            return

        chuoi = message.text.split("|")
        gia = int(chuoi[0].strip())
        thong_tin = chuoi[1].strip()
        
        if gia <= 0:
            bot.reply_to(message, "❌ Giá tiền phải lớn hơn 0 VNĐ!")
            return
        
        data = doc_data()
        
        # Chỉ lấy danh sách các acc còn hàng hoặc đã bán để tính toán ID tiếp theo
        ...
        ghi_data(data)
        bot.reply_to(message, f"✅ Đã thêm thành công sản phẩm *ACC #{new_id}* vào kho hàng!\n💰 Giá: {gia:,} VNĐ", parse_mode="Markdown")
    except Exception as e:
        bot.reply_to(message, "❌ Lỗi hệ thống khi xử lý chuỗi! Hãy chắc chắn nhập đúng mẫu: `Giá | Thông tin`")
...
def callback_xu_ly(call):
    ...
    # --- NHÓM CALLBACK ĐIỀU KHIỂN CỦA ADMIN ---
    if call.data == "admin_add_acc":
        if call.from_user.id != ADMIN_ID: return
        msg = bot.send_message(call.message.chat.id, "📝 Vui lòng nhập thông tin acc theo đúng định dạng:\n`Giá | Tài khoản và mật khẩu`\n\nVí dụ: `25000 | TK: dangvip1 | Pass: 12345`", parse_mode="Markdown")
        ...
        for acc in data["accounts"]:
            if acc["status"] == "ConHang":
                co_acc = True
                markup.add(types.InlineKeyboardButton(f"❌ Xoá Acc #{acc['id']} - Giá: {acc['price']:,}đ", callback_data=f"adm_delete_{acc['id']}"))
        
        if co_acc:
            bot.send_message(call.message.chat.id, "🛠 Chọn tài khoản bạn muốn xoá bỏ hoàn toàn khỏi hệ thống kho:", reply_markup=markup)
        else:
            bot.send_message(call.message.chat.id, "📭 Hiện tại kho hàng đang trống, không có tài khoản nào để xoá.")
        bot.answer_callback_query(call.id)
        ...
        bot.send_message(call.message.chat.id, f"🗑 Đã xoá bỏ hoàn toàn tài khoản *ACC #{acc_id}* ra khỏi cơ sở dữ liệu.", parse_mode="Markdown")
        bot.answer_callback_query(call.id, "Đã xoá thành công!")

    # --- NHÓM CALLBACK CỦA NGƯỜI DÙNG ---
